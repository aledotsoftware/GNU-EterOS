...
void omni_fill_gradient_v(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t color_top, uint32_t color_bottom) {
    ...
    if (omni_bpp == 32) {
        /* Fixed-Point 16.16 Optimization:
           Avoids per-row division and handles negative deltas correctly. */
        int32_t r_val = tr << 16;
        int32_t g_val = tg << 16;
        int32_t b_val = tb << 16;

        /* Calculate steps: (delta << 16) / h */
        /* Cast to int32_t ensures signed subtraction for negative gradients */
