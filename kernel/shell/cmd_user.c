...
static void cmd_useradd(const char* args) {
    ...
    if (shadow_node) {
        ...
        uint32_t offset = 0;
        while (1) {
            int bytes = read_line_vfs(shadow_node, offset, line, sizeof(line));
            if (bytes <= 0) break;
            offset += bytes;
    ...
    // Manual strcpy
    for (int i = 0; username[i] != '\0' && len < MAX_LINE - 1; i++) entry[len++] = username[i];
    if (len < MAX_LINE - 1) entry[len++] = ':';
    for (int i = 0; hash_str[i] != '\0' && len < MAX_LINE - 1; i++) entry[len++] = hash_str[i];
    if (len < MAX_LINE - 1) entry[len++] = ':';
    for (int i = 0; uid_str[i] != '\0' && len < MAX_LINE - 1; i++) entry[len++] = uid_str[i];
    if (len < MAX_LINE - 1) entry[len++] = ':';
    for (int i = 0; uid_str[i] != '\0' && len < MAX_LINE - 1; i++) entry[len++] = uid_str[i];
    if (len < MAX_LINE - 1) entry[len++] = '\n';
    entry[len] = '\0';

    write_fs(shadow_node, shadow_node->length, strlen(entry), (uint8_t*)entry);
...
static void cmd_userdel(const char* args) {
    ...
    if (!tmp_node) {
        terminal_write_string("  Error: No se pudo crear temporal.\n");
        kfree(buffer);
        return;
    }
...
static void cmd_passwd(const char* args) {
    ...
    while (*tptr) {
        ...
        char uid_str[16] = {0};
        char gid_str[16] = {0};
        int field = 0, c_idx = 0, u_idx = 0, g_idx = 0;
        ...
        if (strcmp(user, username) == 0) {
            ...
            if (e_len < MAX_LINE - 1) entry[e_len++] = ':';
            for (int i = 0; uid_str[i] != '\0' && e_len < MAX_LINE - 1; i++) entry[e_len++] = uid_str[i];
            if (e_len < MAX_LINE - 1) entry[e_len++] = ':';
            for (int i = 0; gid_str[i] != '\0' && e_len < MAX_LINE - 1; i++) entry[e_len++] = gid_str[i];
            if (e_len < MAX_LINE - 1) entry[e_len++] = '\n';
            entry[e_len] = '\0';
