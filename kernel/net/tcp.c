...
static int tcp_send_packet(socket_entry_t* sock, const void* payload, int len, int flags) {
    uint8_t buffer[1514];
    size_t headers_len = sizeof(struct ethernet_header) + sizeof(struct ip_header) + sizeof(struct tcp_header);

    /* Bounds check to prevent stack buffer overflow */
    if (len < 0 || (size_t)len > sizeof(buffer) - headers_len) {
        hal_console_write("[TCP] Error: Packet too large\n");
        return -1;
    }
