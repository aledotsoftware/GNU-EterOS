...
static __attribute__((unused)) int timespec_to_ms(const struct timespec* ts, uint64_t* out_ms) {
    uint64_t sec_ms;
    uint64_t nsec_ms;
...
static __attribute__((unused)) void ms_to_timespec(uint64_t ms, struct timespec* ts) {
    if (!ts) return;
...
static __attribute__((unused)) uint32_t timerfd_write_fs(fs_node_t* node, uint32_t offset, uint32_t size, uint8_t* buffer) {
    (void)node; (void)offset; (void)size; (void)buffer;
    return (uint32_t)-EINVAL;
}

static __attribute__((unused)) int timerfd_ioctl_fs(fs_node_t* node, int request, void* arg) {
    timerfd_ctx_t* tfd;
...
static __attribute__((unused)) void timerfd_close_fs(fs_node_t* node) {
    timerfd_ctx_t* tfd;
    if (!node) return;
    tfd = (timerfd_ctx_t*)node->ptr;
    if (tfd) {
...
static __attribute__((unused)) int timerfd_from_fd(int fd, task_t* current, fs_node_t** out_node, timerfd_ctx_t** out_ctx) {
    fs_node_t* node;
    timerfd_ctx_t* tfd;
