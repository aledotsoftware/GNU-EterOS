...
## 2026-03-03 - [File Descriptor Table UAF/TOCTOU]
**Vulnerability:** File descriptors in a process's `fd_table` were manipulated concurrently across threads without synchronization. If one thread called `sys_close` (freeing the descriptor's VFS node) while another called `sys_read`, `sys_write`, or `sys_dup2`, the `fs_node_t` structure could be freed while still in use, causing a Use-After-Free (UAF) or Time-Of-Check to Time-Of-Use (TOCTOU) vulnerability leading to kernel panic.
**Learning:** Concurrent access to file descriptor tables must be explicitly synchronized. Validating a pointer `if (!fd_table[i].node) return` is useless if a concurrent thread can set it to NULL and free the underlying memory immediately afterward.
**Prevention:** Introduce a spinlock (`fd_lock`) in the `task_t` struct to guard all accesses to the `fd_table`. Use a reference counting mechanism (`ref_count`) on the `fs_node_t` to ensure the structure isn't freed as long as any syscall is currently utilizing it. Temporarily reserve FD slots using a dummy value (`(fs_node_t*)1`) during lengthy creations to prevent race conditions without holding the lock for extended periods.
