...
## 2025-03-05 - Fix partial success handling in sys_readv and sys_writev
**Vulnerability:** `sys_readv` and `sys_writev` syscalls discarded successfully processed bytes when an error occurred midway through an `iovec` array, returning only the negative error code instead of the total bytes processed.
**Learning:** In compound syscalls like vector I/O, POSIX semantics require returning the number of successful bytes if the failure happens after at least some data is transferred. Returning an error code blindly can cause state mismatch and data loss for userspace applications.
**Prevention:** When looping over batch operations, maintain a `total` counter and check it before propagating an error: `return total > 0 ? total : error_code;`
