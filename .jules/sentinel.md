...
## 2026-05-30 - Buffer Overflow via sprintf in userspace libc
**Vulnerability:** The userspace `libc` implemented `sprintf` by internally bounding it to 64KB using `vsnprintf`. However, if the user-provided destination buffer was smaller than the generated string, a stack or heap buffer overflow would still occur because the true size of the target buffer was unknown to `sprintf`.
**Learning:** Wrapping inherently unsafe functions (like `sprintf` or `strcpy`) with large, arbitrary maximum limits (e.g., 64KB) is an inadequate defense mechanism. The API itself must require the caller to provide the exact size of the destination buffer to guarantee memory safety.
**Prevention:** Completely remove unsafe functions like `sprintf` and `vsprintf` from the standard library APIs. Force all userspace code and tests to use explicit bounds-checking alternatives like `snprintf` and `vsnprintf`.
