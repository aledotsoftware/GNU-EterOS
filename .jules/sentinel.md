## 2025-05-23 - Unsafe String Conversions in Kernel
...
## 2025-05-24 - Integer Overflow in Heap Allocation
...
## 2025-05-25 - Missing User Pointer Validation in Syscalls
...
## 2025-05-26 - Recursive Pointer Validation in Scatter/Gather Syscalls
...
## 2025-05-27 - [Stale Data Reuse in mmap MAP_FIXED]
...
## 2025-05-28 - XSS in Internal UI Functions
...
## 2025-05-29 - Implicit Pointer Types in IOCTL
...
## 2025-03-01 - [sys_access Permission Bypass]
...
## 2025-03-03 - [TCP Data Offset Integer Underflow]
...
## 2025-06-02 - Missing User Pointer Validation in sys_wait4
...
## 2025-06-02 - Heap Allocation for Kernel Path Buffers
...
## 2024-05-19 - Enforce Permissions for VFS File Creations
**Vulnerability:** Path-based filesystem creation syscalls (sys_openat with O_CREAT, sys_mkdirat, sys_unlinkat) and file access check (sys_faccessat) lacked complete or secure permission checks. While VFS resolved nodes, it didn't verify if the calling process (UID/GID) had sufficient write access to the parent directory before allowing operations like file creation, deletion, or directory creation. Furthermore, sys_faccessat didn't utilize the standard check_node_permission logic, relying on custom, potentially flawed implementation.
**Learning:** In a monolithic or monolithic-like VFS architecture, passing a VFS lookup is insufficient for file modification. We must explicitly verify permissions against the parent directory's access mask (using check_node_permission with W_OK and X_OK) before permitting new node creation or unlinking.
**Prevention:** Consistently apply check_node_permission across all VFS mutating and inspecting syscalls, particularly those accepting paths instead of file descriptors, and verify parent directory permissions prior to child node creation/deletion.
## 2024-05-24 - Synchronization of Effective User/Group IDs in sys_setuid and sys_setgid
...
## 2025-06-03 - Extensive Heap Allocation for Kernel Path Buffers
