...
## 2026-12-05 - [Bresenham Redundancy for Orthogonal Lines]
**Learning:** Utilizing Bresenham's line algorithm for purely horizontal or vertical lines introduces significant overhead (redundant error calculations, per-pixel bounds checks, and function calls). Furthermore, drawing rectangles by calling a generic line function 4 times compounds this overhead and tracks dirty areas redundantly.
**Action:** Always implement fast paths in line-drawing routines to intercept horizontal (`y0 == y1`) and vertical (`x0 == x1`) lines, redirecting them to highly optimized memory block fill operations (`gfx_fill_rect`). Similarly, construct primitive shapes (like hollow rectangles) directly from these block fill operations rather than generic line functions.
