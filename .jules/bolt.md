...
## 2026-03-06 - [Bulk Memset for String Padding]
**Learning:** String formatting functions (like `snprintf` or `kprintf`) often spend a disproportionate amount of time generating alignment padding (e.g., spaces or zeros) when using character-by-character append loops (`while (pad--) append()`), resulting in excessive bounds checking and array accesses.
**Action:** When adding fixed-character padding, use bulk memory operations (`memset`) combined with a single bounds check. This provides massive performance improvements (up to 30,000x for large widths) and significantly speeds up kernel logging and formatted UI rendering.
