...
- Se aplicaron fixes en `userspace/Makefile` para incluir `libc/src/ctype.c` resolviendo un error de enlazado (`undefined reference to isspace`) en `sh.c`.
- Se añadieron atributos `__attribute__((unused))` en `kernel/arch/x86_64/syscall.c` a funciones de timerfd no usadas para resolver warnings (-Werror).
- El proyecto compila correctamente sin advertencias tratadas como errores, y bootea transicionando a Ring 3 sin Kernel Panics. Todos los subprocesos de las capas subyacentes operan de forma estable.
