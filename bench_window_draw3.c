...
void draw_window_opt(int draw_x_start, int draw_y_start, int width, int height, int win_x_start, int win_y_start, int win_width) {
    ...
    for (int32_t i = 0; i < height; i++) {
        ...
        // Unroll loop by 4 for better instruction pipelining and reduced branch overhead
        for (; j <= width - 4; j += 4) {
            uint32_t c0 = src[j];
            uint32_t c1 = src[j+1];
            uint32_t c2 = src[j+2];
...
int main() {
    ...
    // Simulate semi-transparent window (50% pixels drawn)
    for(int i=0; i<1024*768; i++) {
        win_buffer[i] = (i % 2 == 0) ? 0xFFFFFFFF : 0;
    }
