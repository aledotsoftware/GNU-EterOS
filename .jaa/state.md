...
- [AETHER-LINUX-SUBSYSTEM] Hardened ELF loader `PT_INTERP` boundaries in `kernel/fs/elf.c` to gracefully truncate overflowing interpreter paths, averting buffer overflow risks. Improved memory safety in ABI compatibility layers by enforcing string bounds checking with `vmm_check_user_string` in `sys_readlinkat` and `sys_openat`, fixing latent pointer vulnerabilities. Hardened `sys_arch_prctl` with explicit limits against `USER_BASE` to prevent arbitrary manipulation of kernel-level segment registers (MSRs) via user thread local storage paths (TLS/FS/GS bases). Augmented Linux ABI `mmap` fallback to automatically grant `MAP_PRIVATE` flags for purely anonymous memory allocations where userspace libc omits them.
