...
- **graphics-power-panel-bot**: Expanded DRM abstraction with KMS support (`GETCONNECTOR`, `GETENCODER`, `GETCRTC`, `SETCRTC`, `ADDFB`). Enabled framebuffer memory mapping (`mmap`) via `/dev/dri/card0` in `sys_mmap`. Fixed branding inconsistency by renaming 'Eter OS' to 'eterOS' in web UI code to match official branding guidelines, resolving failing automated verification test (`verify_splash_a11y.py`).
