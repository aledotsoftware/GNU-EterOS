...
#include "../kernel/net/core/dhcp_parser.c"

int main() {
    printf("Running test_dhcp_security...\n");

    const struct dhcp_packet* out_dhcp;
    size_t out_dhcp_len;
    uint8_t short_buffer[10] = {0};
    uint32_t xid = 0x12345678;

    printf("Test 1: dhcp_parse_offer with NULL buffer\n");
    int ret = dhcp_parse_offer(NULL, 100, xid, &out_dhcp, &out_dhcp_len);
    if (ret != -1) {
        printf("FAILED: dhcp_parse_offer did not return -1 for NULL buffer\n");
        return 1;
    }

    printf("Test 2: dhcp_parse_offer with short buffer\n");
    ret = dhcp_parse_offer(short_buffer, sizeof(short_buffer), xid, &out_dhcp, &out_dhcp_len);
    if (ret != -1) {
        printf("FAILED: dhcp_parse_offer did not return -1 for short buffer\n");
        return 1;
    }

    printf("All DHCP security tests passed!\n");
    return 0;
}
