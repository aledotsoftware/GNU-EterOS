...
void test_futex_wait_mismatch(void) {
    ...
    uint32_t uaddr = 100;
    int ret = futex_wait(&uaddr, 101, NULL, 0, FUTEX_BITSET_MATCH_ANY); /* Expected 101, but is 100 */
    assert(ret == -EAGAIN);
    printf("PASS\n");
}
...
void test_futex_wait_success(void) {
    ...
    int ret = futex_wait(&uaddr, 200, NULL, 0, FUTEX_BITSET_MATCH_ANY);
...
void test_futex_wake_logic(void) {
    ...
    node->task = &task2;
    node->uaddr = &uaddr;
    node->bitset = FUTEX_BITSET_MATCH_ANY;
    node->next = NULL;

    buckets[idx].head = node;
    task2.state = TASK_BLOCKED;

    int count = futex_wake(&uaddr, 1, 0, FUTEX_BITSET_MATCH_ANY);
