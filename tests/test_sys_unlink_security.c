...
int unlink_called = 0;
int mock_unlink(fs_node_t* parent, char* name) {
    unlink_called = 1;
    return 0;
}
...
int main() {
    printf("Running sys_unlink security test...\n");
    ...
    int64_t ret = sys_unlink("/etc/passwd");

    if (ret == -EACCES || ret == -EPERM) {
        printf("PASSED: sys_unlink denied write access to /etc for non-root user.\n");
        return 0;
    } else if (unlink_called) {
        printf("VULNERABILITY CONFIRMED: sys_unlink allowed non-root to delete /etc/passwd!\n");
        return 1;
    } else {
        printf("FAILED: sys_unlink returned unexpected error %ld\n", ret);
        return 1;
    }
}
