...
void benchmark_strncmp() {
    ...
    /* Limit comparison to size - 1 (full string) or less */
    size_t limit = size;
    ...
    printf("1. Aligned strings (length %zu bytes, limit %zu)...\n", size - 1, limit);

    clock_t start = clock();
    volatile int res = 0;
    for (int i = 0; i < iterations; i++) {
        res += strncmp(str1, str2, limit); /* This calls eteros_strncmp due to macro */
    }
    clock_t end = clock();
    ...
    /* Test where limit stops the comparison early */
    printf("3. Limited comparison (limit = 100)...\n");
    limit = 100;
    start = clock();
    res = 0;
    for (int i = 0; i < iterations * 1000; i++) { /* More iterations for small limit */
        res += strncmp(str1, str2, limit);
    }
    end = clock();
    time_taken = ((double)(end - start)) / CLOCKS_PER_SEC;
