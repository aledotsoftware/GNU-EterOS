...
int main() {
    ...
    printf("truncate returned %ld\n", sys_truncate("/invalid", 100)); assert(sys_truncate("/invalid", 100) == -1); // resolve_path in mock returns -1
    printf("truncate valid returned %ld\n", sys_truncate("/valid", 100)); assert(sys_truncate("/valid", 100) == -1); // no truncate op on node
    ...
    assert(sys_chown("/valid", 500, 500) == -1);
