...
int vfs_normalize_path(char* out_path, int size, const char* path, const char* base_dir) { if (strcmp(path, "/invalid") == 0) return -2; strncpy(out_path, path, size); return 0; }
...
int vmm_strncpy_from_user(char* dst, const char* src, size_t max) { strncpy(dst, src, max); return strlen(src) < max ? strlen(src) : max; }
...
int main() {
    ...
    printf("truncate returned %ld\n", sys_truncate("/invalid", 100)); assert(sys_truncate("/invalid", 100) == -ENOENT); // resolve_path in mock returns -ENOENT (-2)
    printf("truncate valid returned %ld\n", sys_truncate("/valid", 100)); assert(sys_truncate("/valid", 100) == 0); // no truncate op on node
