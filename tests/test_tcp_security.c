...
int main() {
    printf("Running test_tcp_security...\n");
    ...
    printf("Test 1: sys_bind with bad addr\n");
    int64_t ret = sys_bind(0, bad_addr, good_addrlen);
    if (ret != -EFAULT) {
        printf("FAILED: sys_bind did not return -EFAULT for bad addr\n");
        return 1;
    }

    printf("Test 2: sys_connect with bad addr\n");
    ret = sys_connect(0, bad_addr, good_addrlen);
    if (ret != -EFAULT) {
        printf("FAILED: sys_connect did not return -EFAULT for bad addr\n");
        return 1;
    }

    printf("Test 3: sys_getsockname with bad addr\n");
    ret = sys_getsockname(0, bad_addr, &good_addrlen);
    if (ret != -EFAULT) {
        printf("FAILED: sys_getsockname did not return -EFAULT for bad addr\n");
        return 1;
    }

    printf("Test 4: sys_getpeername with bad addr\n");
    ret = sys_getpeername(0, bad_addr, &good_addrlen);
    if (ret != -EFAULT) {
        printf("FAILED: sys_getpeername did not return -EFAULT for bad addr\n");
        return 1;
    }

    printf("All TCP security tests passed!\n");
    return 0;
}
